...
export function parseSigV3(sig, expectedContext) {
...
export function verifySigV3(payload, sig, opts) {
    ...
    if (opts.expectedRPID) {
        ...
        if (!timingSafeEqual(expectedRpHash, gotRpHash))
    ...
    if (!cryptoVerify('sha256', signedData, pub, derSig))
        throw new Error('invalid signature');
    return { signCount, origin: String(clientData?.origin ?? '') };
}
export function buildSigV3FromWebAuthnAssertion(input) {
